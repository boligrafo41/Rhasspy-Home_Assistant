...
class GetEntitiesController extends SendSplitController {
    ...
    async onInput({ message, parsedMessage, send, done, }) {
        ...
        switch (parsedMessage.outputType.value) {
            case const_2.OutputType.Count:
                payload = entities.length;
                break;
            case const_2.OutputType.Split:
                ...
                return;
            case const_2.OutputType.Random: {
                if (entities.length === 0) {
                    noPayload = true;
                    break;
                }
...
_GetEntitiesController_comparatorService = new WeakMap(), _GetEntitiesController_homeAssistant = new WeakMap(), _GetEntitiesController_currentDevice = new WeakMap(), _GetEntitiesController_currentArea = new WeakMap(), _GetEntitiesController_currentFloor = new WeakMap(), _GetEntitiesController_instances = new WeakSet(), _GetEntitiesController_getEntities = async function _GetEntitiesController_getEntities(conditions, message) {
    ...
    const entities = __classPrivateFieldGet(this, _GetEntitiesController_homeAssistant, "f").websocket.getEntities();
    ...
    for (const state of Object.values(states)) {
        ...
        if (state === null || state === void 0 ? void 0 : state.last_changed) {
        ...
        for (const rule of sortedConditions) {
            ...
            if (rule.condition === const_1.PropertySelectorType.State ||
                ...
                const value = (0, selectn_1.default)(rule.property, state);
                ...
                for (const labelId of entity.labels) {
                    ...
                    const result = (0, operators_1.simpleComparison)(rule.logic, label[rule.property], await this.typedInputService.getValue(rule.value, rule.valueType, {
                        message,
                        entity: state,
                    }));
                    if (result) {
                ...
                let propertyValue;
                if (rule.condition === const_1.PropertySelectorType.Device) {
                    ...
                    propertyValue =
                        device[rule.property];
                }
                else if (rule.condition === const_1.PropertySelectorType.Area) {
                    ...
                    propertyValue = area[rule.property];
                }
                else if (rule.condition === const_1.PropertySelectorType.Floor) {
                    const floor = __classPrivateFieldGet(this, _GetEntitiesController_instances, "m", _GetEntitiesController_getFloor).call(this, entity);
                    if (!floor) {
                        ruleMatched = false;
                        break;
                    }
                    propertyValue = floor[rule.property];
                }
